...
class Channel(virtual.Channel):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._queue_cycle = []
        self.asynchronousClient = self._get_asynchronous_client()
    ...
    def _disconnect_pools(self):
        if not self._disconnecting_pools:
            ...
            try:
                if self._asynchronous_pool is not None:
                    self._asynchronous_pool.disconnect()
                if self._pool is not None:
                    self._pool.disconnect()
                self._asynchronous_pool = self._pool = None
            finally:
    ...
    def _create_client(self, asynchronous=False):
        if asynchronous:
            return self.asynchronousClient(connection_pool=self.asynchronous_pool)
    ...
    def _get_asynchronous_client(self):
        if redis.VERSION < (2, 4, 4):
            raise VersionMismatch(
                'Redis transport requires redis-py versions 2.4.4 or later. '
                'You have {0.__version__}'.format(redis))

        # asynchronousRedis maintains a connection attribute on it's instance and
        # uses that when executing commands
        # This was added after redis-py was changed.
        class asynchronousRedis(redis.Redis):  # pragma: no cover

            def __init__(self, *args, **kwargs):
                super(asynchronousRedis, self).__init__(*args, **kwargs)
                self.connection = self.connection_pool.get_connection('_')

        return asynchronousRedis
    ...
    @property
    def asynchronous_pool(self):
        if self._asynchronous_pool is None:
            self._asynchronous_pool = self._get_pool(asynchronous=True)
        return self._asynchronous_pool
